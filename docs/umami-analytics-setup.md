...
| `promo_recovered` | code, discount, subtotal, currency | phase 6 | checkout/page.tsx (via `useCartPromoRecovery`) — the basket's code put back because nothing carried it to the checkout |
...
| `payment_method_selected` | method (`card` \| `cod` \| `apple_pay`), delivery_method, total, currency | phase 4 | checkout/page.tsx |
...
| `payment_failed` | order_number, error_message, reason, provider (gateway: `stripe` \| `ziina` \| `paymob`; the chosen method on a failure before an order existed), total, stage (create_order\|create_session), currency | phase 1 + phase 4 | checkout/page.tsx (handleSubmit catch) |
...
| Promo Recovered | `promo_recovered` | **A failure counter, not a success.** Each one is a customer who would have been shown one price in the basket and charged a higher one. Zero is the healthy reading; a rising line means the `sessionStorage` handoff is breaking more often. |
...
| Date | Change |
|---|---|
| 2026-09-25 | **No event added, removed or renamed — `provider` gained a third gateway value, `paymob`.** Paymob is the third card gateway (migration 291). Like Ziina it ships inactive and unconfigured, so production traffic is unchanged; but if it is ever activated, `provider` on `payment_failed`, `payment_retry` and `payment_cancelled`, and `order_completed.payment_provider`, will carry `paymob`. `payment_method_selected.method` is unaffected — the customer still picks `card`, `cod` or `apple_pay`, never a processor. **Dashboard action:** none required; reports segmenting failures by `provider` will simply show a `paymob` bucket once it takes traffic. |
| 2026-09-09 | **No event added or removed — `payment_method_selected.method` gained a third value, `apple_pay`.** When Apple Pay is offered the checkout fires this event with `method: 'apple_pay'` (the in-sheet button), alongside the existing `card` and `cod`. It had shipped as a bare `string` in `analytics.ts` and this reference never learned the value (W10), so a report segmenting the event by `method` silently had no `apple_pay` bucket to find. The code now types `method` as `'card' \| 'cod' \| 'apple_pay'` and `analytics-docs.test.ts` holds that union and the `method (…)` list in the table above in step, so a fourth value cannot be added in code without this row. **Dashboard action:** none required — existing `card`/`cod` segments are unaffected; add an `apple_pay` segment if you want the split. Apple Pay is offered only to eligible Safari/iOS sessions, so expect low volume. |
| 2026-08-20 | **One event added: `promo_recovered`** (`checkout/page.tsx`, via the new `useCartPromoRecovery` hook). Nothing removed or renamed. No funnel changes — it is not a step anybody takes, it is a repair. New goal **Promo Recovered**, which is a *failure* counter despite the name: each event is one customer whose discount would otherwise have been lost between the basket and the checkout. Context: the applied code used to travel in `sessionStorage` and nowhere else, and when that write failed the checkout priced the order without the discount **and submitted it that way** — both `useOrderPreview` and the order gate on `promoDiscount > 0`. On 20 August a basket reading 29.75 with `Discount (NEW) -5.25` became a checkout reading 35.00 with an empty promo field. `carts.promo_code` now holds the code server-side (migration 115) and the checkout re-validates it. Expect this event to be **rare or zero**; read any sustained volume as private-mode or quota-exhausted browsers, and compare it against `promo_applied` for a rate. It deliberately does *not* fire `promo_applied` — the customer applied the code once, in the basket, and counting it twice would inflate every coupon's take. |
