...
| 2026-08-10 | **No event added, removed or renamed — but `phone_verify_succeeded` means something wider now.** `PhoneVerify` asks `/auth/phone-verified` before buying an SMS, and a number already proved inside `PHONE_VERIFICATION_TTL_SECONDS` short-circuits straight to success. So **`phone_verify_succeeded` can now fire with no preceding `phone_verify_sent`**, and funnel 3's step 2→3 will read above 100% for returning customers. Read `phone_verify_sent` as *messages we paid for* and `phone_verify_succeeded` as *numbers proved*; they are no longer the same denominator. Expect `sent` volume to fall outright — a 60s resend cooldown and a 3-send ceiling per panel were added at the same time, after three days of unthrottled resends cost $4.04 with zero completed verifications. A sustained `sent` with a flat `succeeded` is now the signal worth alerting on: that is either SMS not arriving, or someone else spending the budget. |
