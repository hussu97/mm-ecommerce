...
| P3-11 | Caching: keys and invalidation split between routers and services; analytics busted only on web order creation (not webhooks/POS/refunds) | `products.py:121-151`, `orders.py:162` | Co-locate keys + invalidation per domain; declare analytics TTL-only and delete the misleading lone bust |
...
| P3-14 | Migrations: 34+ revisions contain `op.execute` content edits (CMS copy, slug fixes, seeded passwords) that replay on fresh environments | versions 015/016/033/054-056/061/062 | Policy: content edits go in `scripts/`, migrations are schema + structural backfills only |
| P3-15 | Admin auth: `TokenResponse` still types `access_token`/`refresh_token` no caller reads (verify the API doesn't actually return them in the body); no per-route permission gating in UI despite backend catalogue | `admin/lib/types.ts:11-16` | Trim the type; optional thin `middleware.ts` cookie check |
