...
export function RecipeEditor({ ownerId, ownerKind, ownerLabel }: RecipeEditorProps) {
  ...
  const load = useCallback(async () => {
    try {
      const [catalogue, existing] = await Promise.all([
        inventoryApi.items(),
        inventoryApi.versionedRecipe(ownerKind, ownerId),
      ]);
      setItems(catalogue);
      setRecipe(existing);
      ...
      if (error instanceof ApiError && error.status === 404) {
        setRecipe(null);
        setMessage('No recipe yet. Add the first inventory ingredient below.');
      } else {
        setMessage(error instanceof ApiError ? error.message : 'Could not load this recipe.');
      }
    }
  }, [ownerId, ownerKind]);
  ...
  return (
    <section className="mt-6 border-t border-gray-200 pt-5">
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="font-display text-lg text-primary">Recipe</h2>
          <p className="text-xs text-gray-500">{ownerLabel}. Ingredients can only be inventory items.</p>
        </div>
      ...
      <div className="space-y-2 border border-gray-200 p-3">
        ...
        <div className="grid gap-3 pt-2 md:grid-cols-[1fr_150px_auto] md:items-end">
          <Select label="Inventory ingredient" value={ingredientId} onChange={(event) => setIngredientId(event.target.value)} placeholder="Add ingredient" options={items.map((item) => ({ value: item.id, label: `${item.name} · ${item.ingredient_unit}` }))} />
          <Input label="Quantity" type="number" min="0.0001" step="0.0001" value={quantity} onChange={(event) => setQuantity(event.target.value)} />
          <Button size="sm" onClick={() => void addIngredient()} disabled={!ingredientId || busy} loading={busy}>Save draft</Button>
        </div>
      </div>
