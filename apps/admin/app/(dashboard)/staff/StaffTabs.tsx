...
/**
 * The sub-navigation shared by the two Staff screens.
 *
 * Staff and Roles used to be two tabs of one in-page `TabBar`; they are two tabs
 * of one section now. Links, not buttons, because each is its own route, and the
 * active one is decided by the URL. Styling mirrors `LogsTabs`/`AnalyticsTabs`/
 * `TabBar` so it reads as one section.
 *
 * `/staff` is the Staff tab itself (no redirect) and a prefix of `/staff/roles`,
 * so the active tab is the *most specific* href that covers the path —
 * otherwise standing on Roles would light both.
 */
const TABS = [
