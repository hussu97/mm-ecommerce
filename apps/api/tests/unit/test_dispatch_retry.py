...
`test_batch_retry` covers the same question for a run. This is the other half,
and until now there was no other half: `dispatch_due_batches` sweeps batches, an
order that went out on its own is in no batch, and the `packed` transition that
first tried it has already happened. Nothing was ever going to try again.

MM-20260815-001 is why this file exists. Packed 11:21 on 15 Aug, `batch_id`
NULL, `last_error = "Courier is not configured; dispatch this order by hand"` —
an error a deploy fixed at 11:47. It sat there until somebody looked.
"""
