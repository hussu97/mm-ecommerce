...
def _branch(opens="09:00", closes="23:00") -> Branch:
    return Branch(name="Dubai", opening_from=opens, opening_to=closes)
...
def test_an_order_placed_in_hours_may_be_taken_by_the_terminal():
    ...
    assert pos_order_service.may_auto_accept(_order(placed), _branch()) is True
...
def test_an_order_placed_overnight_waits_for_a_person():
    ...
    assert pos_order_service.may_auto_accept(_order(placed), _branch()) is False
...
def test_the_placement_time_decides_and_not_the_moment_of_asking():
    ...
    assert pos_order_service.may_auto_accept(overnight, _branch()) is False


def test_a_branch_trading_past_midnight_is_still_open_at_one():
    late = _branch(opens="09:00", closes="02:00")
...
def test_a_counter_order_is_never_in_question():
    ...
    assert pos_order_service.may_auto_accept(counter, _branch()) is True
