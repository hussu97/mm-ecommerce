...
class Order(Base, UUIDMixin, TimestampMixin):
    ...
    __table_args__ = (
        ...
        # Migration 294: the channel column every report, gate and filter keys
        # on. Unconstrained until then (migration 132 said so); a typo'd source
        # silently fell out of every channel's figures.
        status_vocabulary("orders", "source", OrderSourceEnum),
        # Migration 294: every date-bucketed report reads this (see the column).
        Index("ix_orders_reporting_at", "reporting_at"),
    )

    #: `reporting_at` is computed by Postgres and changes whenever `source`,
    #: `delivered_at` or `promised_at` does. Without RETURNING on UPDATE the ORM
    #: expires it after every flush, and the next read is a lazy load — which
    #: raises MissingGreenlet under asyncpg. Eager defaults fetch it back in
    #: the same statement instead.
    ...
    #: books no courier for an aggregator order (`channels.policy_for` says so)
    ...
    #: When the order last reached `delivered`. Stamped by
    #: `order_lifecycle.transition()` for every channel, on every arrival (an
    #: admin can recover an undelivered or cancelled order back to delivered).
    #:
    #: The status trail (`order_status_events`) is otherwise the one record of
    #: when a status was reached — `order_deliveries.delivered_at` was removed
    #: for exactly that reason. This column exists because `reporting_at` below
    #: needs it, and a generated column cannot read another table.
    delivered_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    #: The moment every date-bucketed report (dashboard, P&L, orders list,
    #: analytics) files this order under. For every channel but one it is
    #: `created_at`. A **custom** order is taken days before it is made, and the
    #: shop books it on the day it is handed over: its delivered date, else the
    #: delivery date it was promised, else when it was taken.
    #:
    #: Generated by Postgres (migration 294) so the rule lives in one place and
    #: is indexable; never written by the ORM.
    reporting_at: Mapped[datetime] = mapped_column(
