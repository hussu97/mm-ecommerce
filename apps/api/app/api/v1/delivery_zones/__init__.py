...
This was one 1,581-line module serving five unrelated resource families behind
one prefix — versions and their zones, batch groups, batch windows, the runs
themselves, couriers, and the shop-wide settings row. Each has its own
`APIRouter` here, so the routes and their prefix are unchanged.
"""

from fastapi import APIRouter

from . import batch_groups, batch_windows, batches, couriers, settings, versions

router = APIRouter()
# Include order does not affect matching here: every route in every group
# begins with a distinct literal segment (`/versions`, `/batch-groups`,
# `/batch-windows`, `/batches`, `/couriers`, `/settings`, `/map`, `/polygons`,
# `/summary`) and none of the groups declares a bare `/{param}`. Adding one
# would change that, so add it to `versions` and keep it last.
router.include_router(versions.router)
router.include_router(batch_groups.router)
router.include_router(batch_windows.router)
