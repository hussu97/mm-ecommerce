...
| `AGGREGATOR_SALES_REFRESH_MINUTES` | `0` | The rolling sales-only refresh cadence — separate from the daily pass. **Disabled (`0`) after the 2026-08-30 incident**: the sweep holds a DB connection + the sales advisory lock across the per-channel reauth wait, so a dead Careem session pinned connections each tick and exhausted the pool, taking the API down. Re-enable only after the sweep releases its connection during the reauth wait. `0` = daily only |
