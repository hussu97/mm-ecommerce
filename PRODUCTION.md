...
| `TWOCAPTCHA_API_KEY` | (unset) | Optional 2captcha key for Careem reCAPTCHA v2 image grids on **aggregator-worker** (not the API). Empty: the worker still attempts the audio challenge. GitHub secret |
...
| `AGGREGATOR_SALES_REFRESH_MINUTES` | `60` | The rolling sales-only refresh cadence — separate from the daily pass. Every N minutes it re-scrapes only sales over a short rolling window and re-promotes, so late-settling values (a Talabat commission landing hours later) are caught within the hour. Re-enabled at 60 after the 2026-08-30 pool-exhaustion incident was fixed (the sweep now releases its DB connection before the per-channel reauth wait). `0` = daily only |
...
| `AGGREGATOR_COVERAGE_BACKFILL_DAYS` | `7` | Standing multi-day coverage backfill. The daily pass looks back only `AGGREGATOR_LOOKBACK_DAYS` and the rolling refresh only `AGGREGATOR_SALES_ROLLING_HOURS`, so a session outage longer than either leaves the business dates it spanned permanently uncovered — the recurring "2-3 aggregator orders short vs the portal" hole. This pass re-pulls any still-uncovered date in the last N days once the session is live again (idempotent, SALES only; skips push-only Keeta and never wakes a dead login). `0` = disable |
| `AGGREGATOR_COVERAGE_BACKFILL_MINUTES` | `60` | How often that backfill checks for gaps. Cheap query + idempotent re-pull, so hourly is ample. `0` = disable (safety valve) |
| `AGGREGATOR_AUTODELIVER_STALE_HOURS` | `8` | Safety net: book any aggregator order stuck in `out_for_delivery` longer than N hours to `delivered`. The delivered rung comes only from the channel scrape (the GrubOps live push stops at out_for_delivery), so a missed scrape row (e.g. Talabat's export dropping a block of orders, 2026-09-17) strands an order forever. 8h is well past any real delivery. `0` = disable |
...
| `TALABAT_ORDER_DETAIL_ENRICH` | `true` | Overlay real per-line prices onto direct-Talabat orders. The Report Builder CSV omits per-line money, so a **multi-item** Talabat order lands with every line at 0 (header total still correct); when on, the sales sweep fetches `ListOrders` (for the exact placed timestamp) + `GetOrderDetails` and rewrites the lines, guarded by a reconciliation to the order total. Read-only against the portal, fail-soft (any error keeps the CSV line). Only affects Al Karama — Foodics branches (Sharjah/Barsha) are rebuilt from the GrubOps push |
